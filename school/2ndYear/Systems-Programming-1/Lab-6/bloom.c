...
int bloom_lookup(struct bloom * this, char * item)
{
  ...
  int hashedItem = hash_string(item, BLOOM_HASH1, this->size);

  if(bitset_lookup(this->bitset, hashedItem) == 0)
  {
    return 0;
  }

  hashedItem = hash_string(item, BLOOM_HASH2, this->size);
...
void bloom_add(struct bloom * this, char * item)
{
  ...
  bitset_add(this->bitset, hashedItem);
