...
char ** changeInfixToPostfix(char ** inputs, int numberOfInputs)
{
	...
	{
		if(strcmp(inputs[j], "+") != 0 && strcmp(inputs[j], "-") != 0 && strcmp(inputs[j], "x") != 0 && strcmp(inputs[j], "/") != 0 && strcmp(inputs[j], "^") != 0)
		{
		...
		else if(strcmp(inputs[j], "+") == 0 || strcmp(inputs[j], "-") == 0 || strcmp(inputs[j], "x") == 0 || strcmp(inputs[j], "/") == 0 || strcmp(inputs[j], "^") == 0)
		{
			...
			if(strcmp(inputs[j], "+") == 0 || strcmp(inputs[j], "-") == 0)
			{
				if(this->top != 0)
				{
					while( strcmp( (this->items[this->top]) , "x") == 0 || strcmp( (this->items[this->top]) , "/") == 0 || strcmp( (this->items[this->top]) , "^") == 0 )	// BREAKS HERE
					{
						printf("d\n");
		...
		{
			while (strcmp(this->items[this->top-1], "(") != 0)
			{
				addToOutput = stack_pop(this);
				outputString[i] = addToOutput;
				i++;
...
double evaluate_infix_expression(char ** inputs, int numberOfInputs)
{
	char ** postfixInputs = changeInfixToPostfix(inputs, numberOfInputs);
	int i;
